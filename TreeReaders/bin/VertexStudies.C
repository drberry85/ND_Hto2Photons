      ...
      for ( Long64_t i = 0; i < nentries; i++ ) {
        ...
        */
        ...
        for ( unsigned int j = 0; j < (unsigned int) currentTree.pho_n; j++) {
          ...
          if (fabs(Photonxyz[j].Eta())>2.5 ) continue; 

          // CiC selection as in Hto2Photons
          if (currentTree.pho_cic4cutlevel_lead->at(j)<4 ) continue;
          int iConv = gettrackerconvindex(&currentTree,Photonxyz[j]);
          if (iConv <0 ) continue;

          double deltaeta = Photonxyz[j].Eta() - etaTransformation(ConversionRefittedPairMomentum[iConv].Eta(),currentTree.conv_zofprimvtxfromtrks[iConv]);
          double deltaphi = DeltaPhi(Photonxyz[j].Phi(),ConversionRefittedPairMomentum[iConv].Phi());
          double EoP = MatchedSuperCluster.E()/ConversionRefittedPairMomentum[iConv].Mag();
          histoContainer->Fill("convdphi",PhotonDetector,deltaphi,weight);
          histoContainer->Fill("convdeta",PhotonDetector,deltaeta,weight);
          double zConv  = FindNewZConvLinear(ConversionVertex[iConv],Photonxyz[j],PrimaryVertex[0]);
          string iConvDetector = DetectorPosition(&currentTree, j);

          if (iConvDetector=="Barrel") {

            if ( ConversionVertex[iConv].Perp() <=15 )                                          
              histoContainer->Fill("dZPix",iConvDetector, zConv-PrimaryVertex[0].Z(),weight);
            else if (ConversionVertex[iConv].Perp()>15 && ConversionVertex[iConv].Perp()<=60)  
              histoContainer->Fill("dZTib",iConvDetector, zConv-PrimaryVertex[0].Z(),weight);
            else if (ConversionVertex[iConv].Perp()>60 )  
              histoContainer->Fill("dZTob",iConvDetector,  zConv-PrimaryVertex[0].Z(),weight);
          ...
          if (iConvDetector=="Endcap") {
            ...
            if ( fabs(zConv) <=50  )                                          
              histoContainer->Fill("dZPix",iConvDetector, zConv-PrimaryVertex[0].Z(),weight);
            else if ( fabs(zConv) > 50 && zConv <= 100 )  
              histoContainer->Fill("dZTid",iConvDetector, zConv-PrimaryVertex[0].Z(),weight);
            else if ( fabs(zConv) > 100 )  
              histoContainer->Fill("dZTec",iConvDetector,  zConv-PrimaryVertex[0].Z(),weight);
...
void BookHistograms(HistoContainer *histoContainer) {
  ...
  histoContainer->Add("dZPixBarrel","",100,-5.,5.);
  histoContainer->Add("dZTibBarrel","",100,-5.,5.);
  histoContainer->Add("dZTobBarrel","",100,-5.,5.);
  histoContainer->Add("dZPixEndcap","",100,-5.,5.);
  histoContainer->Add("dZTidEndcap","",100,-5.,5.);
  histoContainer->Add("dZTecEndcap","",100,-5.,5.);
