void fitter (char * filename = "/data/ndpc2/c/HiggsGammaGamma/CMSSW_3_8_5_patch3/src/ND_Hto2Photons/TreeReaders/UnweightedHiggsAnalysis120GeV.root", float startmass = 120) {
  ...
  for (Int_t i=1; i<HistList->GetSize(); ++i) {
    ...
    if (HistName.Contains("mass_2gamma")==1) {
      ...
      //fracG.setConstant();
      //RooFFTConvPdf ResolutionModel("ResolutionModel","Resolution Model", mass, BreitWigner, CrystalBall);
      ...
      ResolutionModel.paramOn(plot,Format("NEL",FixedPrecision(4)), Parameters(RooArgSet(m0, sigma, cut, power, GaussianSigma, fracG)), Layout(.15, 0.5, 0.9), ShowConstants(kFALSE));
